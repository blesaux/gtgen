%% paintMaskNd.m
% A function for generating gound-truth masks for multi-spectral images
%
...
function [mask] = paintMaskNd(dat, mask)

bandNumbers = [1:size(dat,3)];
bandString = strtrim(cellstr(num2str(bandNumbers'))');
if size(dat,3) >= 3
    band1 = 1;
    band2 = 2;
    band3 = 3;
...
h17 = uicontrol('Style', 'popup', ...
     'Position', [80 700 50 30], ...
     'String',bandString,...
     'Value',band1,...
     'Tag','h17',...
     'Callback', @updateDataBands);
h18 = uicontrol('Style', 'popup', ...
     'Position', [130 700 50 30], ...
     'String',bandString,...
     'Value',band2,...
     'Tag','h18',...
     'Callback', @updateDataBands);
h19 = uicontrol('Style', 'popup', ...
     'Position', [180 700 50 30], ...
     'String',bandString,...
     'Value',band3,...
     'Tag','h19',...
     'Callback', @updateDataBands); 
h20 = uicontrol('Style', 'pushbutton', ...
     'Position', [254 705 66 50], ...
     'String','refresh',...
     'Callback', @refreshDataBands); 
h21 = uicontrol('Style','text',...
        'Position',[80 735 150 20],...
        'String','Bands displayed');
...
% Store gui object
set(gcf,'UserData',{gui;mask;maskedim;ima;dat});
...
% Store gui object
ud{2}=mask;
ud{3}=maskedim;% ima, dat, gui are not modified..
set(gcf,'UserData',ud);
...
pan;% something to do for coming back ?

%% update the selected bands for visu
function updateDataBands(src,evnt) %#ok<INUSD>
%does nothing

%% refresh data visu with the selected bands
function refreshDataBands(src,evnt) %#ok<INUSD>
% get new band values
hr=findobj(gcf,'Style','popup','-and','Tag','h17');
band1=get(hr,'Value');
hr=findobj(gcf,'Style','popup','-and','Tag','h18');
band2=get(hr,'Value');
hr=findobj(gcf,'Style','popup','-and','Tag','h19');
band3=get(hr,'Value');
